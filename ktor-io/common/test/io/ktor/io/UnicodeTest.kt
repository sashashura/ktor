...
class UnicodeTest {
    @Test
    fun smokeTest() {
        val packet = buildPacket { writeString(smokeTestData) }.toByteArray()
        assertEquals(smokeTestDataAsBytes.hexdump(), packet.hexdump())
    }

    @Test
    fun testInputEncoding() {
        val packet = buildPacket { writeString(testData) }.toByteArray()
    ...
    fun testInputDecoding() {
        val text = buildPacket { writeByteArray(testDataAsBytes) }.readString()
    ...
    companion object {
        ...
        private val testDataAsBytes: ByteArray = (
            "66 69 6c 65 20 63 6f 6e 74 65 6e 74 20 77 69 74 " +
