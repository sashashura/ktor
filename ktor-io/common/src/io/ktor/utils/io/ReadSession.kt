 ...
 * should never capture a provided [DROP_Memory] instance outside otherwise an undefined behaviour may occur including
...
public suspend inline fun ByteReadChannel.read(
    desiredSize: Int = 1,
    block: (source: DROP_Memory, start: Long, endExclusive: Long) -> Int
): Int {
    val buffer = requestBuffer(desiredSize) ?: DROP_Buffer.Empty
...
public interface ReadSession {
    ...
    public fun request(atLeast: Int = 1): DROP_ChunkBuffer?
}
...
@PublishedApi
internal suspend fun ByteReadChannel.requestBuffer(desiredSize: Int): DROP_Buffer? {
    @Suppress("DEPRECATION")
    ...
    if (readSession != null) {
        val buffer = readSession.request(desiredSize.coerceAtMost(DROP_Buffer.ReservedSize))
        if (buffer != null) {
            return buffer
        }
...
@PublishedApi
internal suspend fun ByteReadChannel.completeReadingFromBuffer(buffer: DROP_Buffer?, bytesRead: Int) {
    ...
    if (buffer is DROP_ChunkBuffer && buffer !== DROP_ChunkBuffer.Empty) {
...
@Suppress("DEPRECATION")
private suspend fun SuspendableReadSession.requestBufferSuspend(desiredSize: Int): DROP_Buffer? {
    await(desiredSize)
    return request(1)
}

private suspend fun ByteReadChannel.requestBufferFallback(desiredSize: Int): DROP_ChunkBuffer {
    val chunk = DROP_ChunkBuffer.Pool.borrow()
    val copied =
